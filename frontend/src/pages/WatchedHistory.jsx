...
function WatchedHistory() {
  ...
  return (
    <div className="watched-history-page" style={{ padding: "10px 0" }}>
      <h1 style={{ fontSize: "36px", marginBottom: "10px" }}>👁️ Watched History</h1>
      <p style={{ color: "#94a3b8", marginBottom: "25px" }}>A timeline of all the movies you have watched.</p>

      {/* Loading state */}
      {loading && (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          <div className="spinner" style={{ border: "4px solid rgba(255,255,255,0.1)", borderTop: "4px solid #10b981", borderRadius: "50%", width: "40px", height: "40px", animation: "spin 1s linear infinite", margin: "auto" }} />
          <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
          <p style={{ marginTop: "15px", color: "#cbd5e1" }}>Loading watched history...</p>
        </div>
      )}
        ...
        <>
          {watchedMovies.length === 0 ? (
            <div className="empty-watched" style={{ textAlign: "center", padding: "60px 20px", background: "#1e293b", borderRadius: "15px", color: "#cbd5e1" }}>
              ...
              <h3>No watched movies recorded.</h3>
              <p style={{ color: "#94a3b8", marginTop: "5px" }}>Start marking movies as watched from search results, watchlist, or recommendations!</p>
            </div>
          ) : (
            <>
