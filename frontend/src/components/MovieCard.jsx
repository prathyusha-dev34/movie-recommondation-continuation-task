...
function MovieCard({ movie }) {
  ...
  const addToFavorites = async () => {
    try {
      ...
      showToast(response.data.message || "Added to Favorites ❤️");
    } catch (error) {
      console.error(error);
      showToast(
  ...
  const addToWatchlist = async () => {
    try {
      ...
      showToast(response.data.message || "Added to Watchlist 📺");
    } catch (error) {
      console.error(error);
      showToast(
  ...
  const toggleWatched = async () => {
    try {
      if (isWatched) {
        ...
        showToast("Removed from Watched History 👁️");
      } else {
        const watchedData = {
          movie_id: String(movieId),
          movie_title: movieTitle,
  ...
  const addReview = async () => {
    try {
      ...
      showToast(response.data.message || "Review Added ⭐");
    } catch (error) {
      console.error(error);
      showToast(
  ...
  const getReviews = async () => {
    try {
      ...
      if (response.data.reviews.length === 0) {
        showToast("No reviews found", "error");
        return;
      }
      ...
      alert(reviewsText); // Keep alert here for displaying full review text
    } catch (error) {
      console.error(error);
